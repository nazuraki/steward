...
export function loadRepoConfig(workDir: string): RepoConfig {
	...
	if (process.env.REQUIRE_ISSUE_ALLOWLIST !== undefined)
		envOverrides.requireIssueAllowlist =
			process.env.REQUIRE_ISSUE_ALLOWLIST !== "false";
	...
	if (rateLimitThreshold !== undefined)
		envOverrides.rateLimitThreshold = rateLimitThreshold;

	const toolIterations = envInt("MAX_TOOL_ITERATIONS");
	if (toolIterations !== undefined)
		limitsOverrides.toolIterations = toolIterations;

	const prFixIterations = envInt("MAX_PR_FIX_ITERATIONS");
	if (prFixIterations !== undefined)
		limitsOverrides.prFixIterations = prFixIterations;
	...
	if (runtimeSeconds !== undefined)
		limitsOverrides.runtimeSeconds = runtimeSeconds;
	...
	if (cmdOutputMaxTokens !== undefined)
		limitsOverrides.cmdOutputMaxTokens = cmdOutputMaxTokens;
	...
	if (issueFailures !== undefined)
		limitsOverrides.issueFailures = issueFailures;

	const consecutiveFailureThreshold = envInt("CONSECUTIVE_FAILURE_THRESHOLD");
	if (consecutiveFailureThreshold !== undefined)
		limitsOverrides.consecutiveFailureThreshold = consecutiveFailureThreshold;
