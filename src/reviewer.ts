...
export class Reviewer {
  ...
  async review(issue: IssueSummary, planBody: string, prNumber: number): Promise<ReviewResult> {
    ...
    await this.gh.postIssueComment(prNumber, isClean ? `✅ ${text}` : `⚠️ Self-review flagged issues:\n\n${text}`);
    return isClean ? 'clean' : 'has_issues';
  }
}
...
function buildUserPrompt(issue: IssueSummary, planBody: string, diff: string): string {
  return [
