...
export async function validate(env: EnvConfig): Promise<ValidationResult> {
  ...
  try {
    ...
    if (status === 403) return fail(`Anthropic API access denied: ${message}. Check your credit balance and workspace at console.anthropic.com`);
    if (status === 400) { /* max_tokens=1 may 400 on some models — key is fine */ }
  ...
  try {
    ...
    if (status === 404) return fail(`Repo ${env.githubRepo} not found or token lacks read access`);
  ...
  for (const name of ['ANTHROPIC_API_KEY', 'GITHUB_REPO', 'GITHUB_BOT_USERNAME', 'GIT_USER_NAME', 'GIT_USER_EMAIL', 'WORK_DIR', 'LOG_DIR']) {
  ...
  try {
    ...
    execFileSync(
      'git',
      ['clone', '--depth=1', repoUrl, env.workDir],
      { stdio: 'pipe', timeout: 120_000 },
    );

    // Configure git identity for commits
    execFileSync('git', ['-C', env.workDir, 'config', 'user.name', env.gitUserName], { stdio: 'pipe' });
    execFileSync('git', ['-C', env.workDir, 'config', 'user.email', env.gitUserEmail], { stdio: 'pipe' });
  } catch (e: unknown) {
