...
export class Implementer {
  ...
  async run(issue: IssueSummary, planBody: string): Promise<ImplementResult> {
    ...
    if (!dirty) {
      return { status: 'error', error: 'Only workflow file changes were produced — posted to issue for manual application' };
    }

    // Commit before rebase
    this.git('commit', '-m', `${commitTitle}\n\nCloses #${issue.number}\n[steward run ${this.logger.runId}]`);
    ...
    try {
      ...
      try { this.git('checkout', defaultBranch); } catch { /* best effort */ }
      try { this.git('branch', '-D', branchName); } catch { /* best effort */ }
  ...
  private async toolLoop(
    issue: IssueSummary,
    planBody: string,
  ): Promise<{ status: 'ok'; lastTestOutput: string } | { status: 'budget_exceeded' } | { status: 'error'; error: string }> {
    ...
    const { toolIterations, commands: maxCommands, fileWrites: maxFileWrites, tokensPerRun } = this.config.limits;
    let commandCount = 0;
    ...
    for (let iteration = 0; iteration < toolIterations; iteration++) {
      ...
      this.logger.addTokens(response.usage.input_tokens, response.usage.output_tokens);

      if (tokensPerRun !== null && this.logger.getTotalTokens() > tokensPerRun) {
        ...
        switch (block.name) {
          ...
          case 'write_file':
            if (fileWriteCount >= maxFileWrites) {
              ...
              result = writeFile(this.env.workDir, String(input.path ?? ''), String(input.content ?? ''));
              fileWriteCount++;
            }
            break;
          ...
          case 'post_comment':
            if (this.env.dryRun) {
              ...
              await this.gh.postIssueComment(issue.number, String(input.body ?? ''));
  ...
  private async extractWorkflowChanges(issue: IssueSummary): Promise<void> {
    ...
    const newFiles = new Set(
      this.gitOutput('diff', '--cached', '--name-only', '--diff-filter=A')
        .split('\n').map(f => f.trim()).filter(f => f.startsWith('.github/workflows/')),
    );
    ...
    for (const f of stagedNames) {
      if (newFiles.has(f)) {
        try { unlinkSync(join(this.env.workDir, f)); } catch { /* already gone */ }
      } else {
        try { this.git('checkout', '--', f); } catch { /* best effort */ }
      }
    }
    ...
    if (this.env.dryRun) {
      process.stderr.write(`[steward] DRY RUN — would post workflow diff to issue #${issue.number}\n`);
    } else {
      await this.gh.postIssueComment(issue.number, body);
    }
  }

  private async generateCommitTitle(issue: IssueSummary, planBody: string): Promise<string> {
    ...
    const description = response.content.find(b => b.type === 'text')?.text?.trim() ?? '';
  ...
  private buildSystemPrompt(contribAgents: string | null, context: string | null): string {
    const { toolIterations, commands: maxCmds, fileWrites: maxWrites } = this.config.limits;
    const allowed = this.config.commands.length > 0 ? this.config.commands.join(', ') : '(none configured)';
    ...
    if (contribAgents) parts.push('', '## Agent contribution guidelines', '', contribAgents);
  ...
  private buildPRBody(issue: IssueSummary, planBody: string, testOutput: string): string {
    // First paragraph of the plan as summary
    const summary = planBody.replace(/^#+\s*/m, '').split(/\n\n/)[0].trim();
  ...
  private gitOutput(...args: string[]): string {
    return execFileSync('git', ['-C', this.env.workDir, ...args], { encoding: 'utf-8', stdio: 'pipe' });
  }
}
...
function conventionalTypeFromLabels(labels: string[]): string {
  ...
  if (lower.some(l => ['bug', 'fix', 'bugfix', 'hotfix'].includes(l))) return 'fix';
  if (lower.some(l => ['documentation', 'docs'].includes(l))) return 'docs';
  if (lower.some(l => ['refactor', 'refactoring'].includes(l))) return 'refactor';
...
const TOOLS: Tool[] = [
  {
    name: 'read_file',
    description: 'Read a file by path relative to the repo root. Returns contents, or an error with similar filenames if not found.',
    input_schema: {
      type: 'object',
  ...
  {
    name: 'write_file',
    description: 'Write or overwrite a file. Parent directories are created automatically. Provide the complete new file contents.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'list_directory',
    description: 'List files and subdirectories. Directories are shown with a trailing /.',
    input_schema: {
      type: 'object',
      properties: { path: { type: 'string', description: 'Directory path relative to repo root (omit for repo root)' } },
      required: [],
    },
  },
  {
    name: 'run_command',
    description: 'Run an allowlisted command in the repo. Use for tests, linters, and build tools.',
    input_schema: {
      type: 'object',
      properties: {
        command: { type: 'string', description: 'Executable name (e.g. "npm")' },
        args: { type: 'array', items: { type: 'string' }, description: 'Arguments list (e.g. ["test"])' },
      },
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        pattern: { type: 'string', description: 'Search string (literal by default)' },
        regex: { type: 'boolean', description: 'Treat pattern as a regular expression' },
        path: { type: 'string', description: 'Glob pattern to limit which files are searched' },
      },
      required: ['pattern'],
    },
  },
  {
    ...
    description: 'Get the title, body, and comments of the issue being implemented.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'post_comment',
    description: 'Post a comment on the current issue. Use only to report a blocker that requires human input.',
    input_schema: {
      type: 'object',
