...
export class Planner {
  ...
  async generateAndPost(issue: IssueSummary): Promise<'posted' | 'failed'> {
    ...
    for (let attempt = 0; attempt <= this.config.limits.planRetries; attempt++) {
    ...
    process.stderr.write(`[steward] Plan for issue #${issue.number}:\n${body}\n`);
  ...
  private buildUserPrompt(issue: IssueSummary, strict: boolean): string {
    ...
    parts.push('Produce an implementation plan for this GitHub issue.', '', '<user-content>');
