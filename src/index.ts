...
async function main(): Promise<void> {
  ...
  process.stderr.write(`[steward] run_id=${logger.runId} repo=${env.githubRepo}\n`);
  ...
  let timeoutHandle: ReturnType<typeof setTimeout> | null = setTimeout(async () => {
    logger.write('error', 'timeout', { error: 'MAX_RUNTIME_SECONDS exceeded before config loaded' });
    process.exit(1);
  }, STARTUP_TIMEOUT_MS);
