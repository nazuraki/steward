...
export class Agent {
  ...
  async run(): Promise<void> {
    ...
    const qualifyingPRs = prs.filter(pr =>
      // Only address PRs that steward opened
      pr.headRef.startsWith('agent/') && (
        pr.threads.some(t => this.threadQualifies(t.comments.map(c => c.author))) ||
    ...
    if (qualifyingPRs.length > 0) {
      ...
      switch (result.status) {
        ...
        case 'budget_exceeded':
          ...
          break;
        case 'error':
          this.logger.write('pr_review', 'error', { pr: pr.number, error: result.error });
          break;
      }
      return;
    }
    ...
    for (const issue of candidates) {
      ...
      switch (result.status) {
        case 'success':
          this.logger.write('implement', 'success', { issue: issue.number, pr: result.prNumber });
          break;
        case 'reviewed':
          this.logger.write('implement_reviewed', 'success', { issue: issue.number, pr: result.prNumber });
          break;
        case 'conflict':
          ...
          break;
        case 'budget_exceeded':
          this.logger.write('implement', 'budget_exceeded', { issue: issue.number });
          break;
        case 'error':
          await this.recordFailure(issue);
          this.logger.write('implement', 'error', { issue: issue.number, error: result.error });
          break;
      }
      return;
    }
  ...
  private async recordFailure(issue: IssueSummary): Promise<void> {
    ...
    const failureLabels = issue.labels.filter(l => /^agent:failures:\d+$/.test(l));
  ...
  private async filterIssues(issues: IssueSummary[]): Promise<IssueSummary[]> {
    ...
    for (const issue of issues) {
      ...
      if (this.config.requiredLabels.length > 0) {
        const hasRequired = issue.labels.some(l => this.config.requiredLabels.includes(l));
      ...
      if (this.config.requireIssueAllowlist && this.config.allowlist.length > 0) {
   ...
   */
  private findPlanComment(issue: IssueSummary): { id: number; body: string; hash: string } | null {
  ...
  private isPlanApproved(
    ...
    for (const comment of issue.comments) {
      ...
      if (requireIssueAllowlist && allowlist.length > 0 && !allowlist.includes(comment.author)) continue;
      if (
