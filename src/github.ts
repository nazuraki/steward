...
export class GitHubClient {
	...
	constructor(token: string, githubRepo: string) {
		...
		if (!owner || !repo)
			throw new Error(`Invalid GITHUB_REPO format: ${githubRepo}`);
		this.owner = owner;
		this.repo = repo;
	...
	async getTokenScopes(): Promise<string[]> {
		const res = await this.rest.repos.get({
			owner: this.owner,
			repo: this.repo,
		});
		const scopesHeader =
			(res.headers as Record<string, string>)["x-oauth-scopes"] ?? "";
		return scopesHeader
			.split(",")
			.map((s) => s.trim())
			.filter(Boolean);
	...
	async getOpenPRsWithUnresolvedThreads(): Promise<PRSummary[]> {
		...
		do {
          ...
          repository(owner: $owner, name: $repo) {
			...
			const prs: GQLResult["repository"]["pullRequests"] =
			...
			for (const pr of prs.nodes) {
				const unresolvedThreads = pr.reviewThreads.nodes
					...
					.map((t: GraphQLReviewThread): ReviewThread => {
						...
						return {
							...
							comments: t.comments.nodes.map(
								(
									c: GraphQLReviewThread["comments"]["nodes"][number],
								): ReviewComment => ({
				...
				const changeRequestedReviews: ReviewSummary[] = pr.reviews.nodes
					.filter(
						(r: GraphQLReview) =>
							r.state === "CHANGES_REQUESTED" && r.body.trim(),
					)
					.map(
						(r: GraphQLReview): ReviewSummary => ({
	...
	async getOpenIssues(priorityLabels: string[]): Promise<IssueSummary[]> {
		...
		return allIssues.sort((a, b) => {
			const tierDiff =
				priorityTier(a.labels, priorityLabels) -
				priorityTier(b.labels, priorityLabels);
