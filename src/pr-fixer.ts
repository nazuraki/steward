...
export class PRFixer {
  ...
  async fix(pr: PRSummary): Promise<PRFixResult> {
    ...
    if (this.env.dryRun) {
      process.stderr.write(`[steward] DRY RUN — skipping commit, push, comment\n`);
  ...
  private async toolLoop(
    pr: PRSummary,
  ): Promise<{ status: 'ok' } | { status: 'budget_exceeded' } | { status: 'error'; error: string }> {
    ...
    const { prFixIterations, commands: maxCommands, fileWrites: maxFileWrites, tokensPerRun } = this.config.limits;
    let commandCount = 0;
        ...
        switch (block.name) {
          ...
          case 'get_pr_feedback':
            result = JSON.stringify({
              ...
              reviews: pr.reviews.map(r => ({ author: r.author, body: r.body })),
            }, null, 2);
            break;
          ...
          case 'post_comment':
            if (this.env.dryRun) {
              ...
              await this.gh.postIssueComment(pr.number, String(input.body ?? ''));
  ...
  private buildSystemPrompt(contribAgents: string | null, context: string | null): string {
    const { prFixIterations, commands: maxCmds, fileWrites: maxWrites } = this.config.limits;
...
const PR_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: 'Get all review feedback on this PR: unresolved inline threads and CHANGES_REQUESTED review comments.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'post_comment',
    description: 'Post a comment on the PR. Use only to report a blocker that requires human input.',
    input_schema: {
      type: 'object',
